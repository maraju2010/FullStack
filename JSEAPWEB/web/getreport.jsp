...
<body>
                ...
                <div class="container-fluid" id="containerflag">
                    ...
                    <h2 class="sub-header placeholders">User Phone Map</h2>
  ...
  <div class="row d-flex justify-content-center modalWrapper" data-backdrop="false">
    ...
    <div class="modal fade addNewInputs" id="modalAdd" tabindex="-1" role="dialog" aria-labelledby="modalAdd"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header text-center">
            <h4 class="modal-title w-100 font-weight-bold text-primary ml-5">Add User-Phone</h4>
            <button type="button" class="close text-primary" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
           <form method="post" action="#" id="singleupload">
    ...
    <div class="text-center">
      <a href="" class="btn btn-success btn-rounded btn-sm" data-toggle="modal" data-target="#modalAdd">Add<i
          class="fas fa-plus-square ml-1"></i></a>
                    ...
                    <table id="DBtable" class="table table-responsive table-hover table-active table-bordered  table-sm display small" style="width:100%" >
     ...
     <!--   for (Todo todo: todos) {  -->
                  <c:set var="count" value="${countervalue}" scope="page" /> 
                    ...
                    <tr class="btnDelete" data-id=${todo.UserId}>
...
$(function() {
 $("#next").click(function(evt) {
  ...
  $("#containerflag").load("<%=request.getContextPath()%>/GetMaxRows?page=${currentpage + 1}" + "&maxrows=" + maxrows);
   evt.preventDefault();
});
}); 
...
$(function() {
 $("[id^=varpage_]").click(function(evt) {
  ...
  $("#containerflag").load("<%=request.getContextPath()%>/GetMaxRows?page=" + selected_id + "&maxrows=" + maxrows );
   evt.preventDefault();
});
});
...
$(function() {
 $("#maxRows").on('change',function(evt) {
  ...
  $(function(){
    $("#singleupload").submit(function(evt){   
        var form = $('#singleupload');
        var result=null;
        
        $.ajax({
        type: form.attr('method'),
        url: "<%=request.getContextPath()%>/SinglePhone",
        data: form.serialize(),
        
        success: function (data) {
            result=data;
        $('#singlestatus').text(result);
         ...
         complete:function(){
            $('#singleupload').each(function(){
                this.reset(); 
                $('#singlestatus').html(result);//Here form fields will be cleared.
                $('#modalAdd').modal('hide');
            });
       }
        });
...
$('button.btnDelete').on('click', function (e) {
  ...
  $(function(){
    $("#btnYes").click(function(evt){   
        ...
        console.log(id);
         $('#singlestatus').load("<%=request.getContextPath()%>/deletephone?UserId=" + id);
         evt.preventDefault();
